...
// OrderResolverService fetches and normalizes a live transaction from Capa.
...
// from Capa, and syncs the result to the legacy database.
...
// from Capa, syncs the result to the legacy database, and returns the resolved state.
func (h *OrderResolveHandler) ResolveOrder(c *fiber.Ctx) error {
	...
	// 3. Fetch live status from Capa and map to canonical model.
	trade, err := h.service.ResolveTransaction(ctx, qrec.ClientID, txID)
	if err != nil {
		h.logger.Error("capa.resolve_order.fetch_failed",
			zap.String("quote_id", quoteID),
			zap.String("tx_id", txID),
			zap.Error(err))
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{"error": "failed to fetch status from Capa"})
	}

	// 4. Sync to legacy database.
	if err := h.tradeSync.SyncTradeUpsert(ctx, trade); err != nil {
		h.logger.Warn("capa.resolve_order.sync_failed",
			zap.String("quote_id", quoteID),
			zap.String("tx_id", txID),
			zap.Error(err))
