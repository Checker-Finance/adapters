...
func (c *CommandConsumer) Subscribe(ctx context.Context, quoteSubject, tradeSubject string) error {
	subQ, err := c.nc.Subscribe(quoteSubject, func(msg *nats.Msg) {
		...
		if err := json.Unmarshal(msg.Data, &env); err != nil {
			slog.Error("capa.cmd.quote_request.unmarshal_failed", "error", err)
			return
		}
		var req model.QuoteRequest
		if err := json.Unmarshal(env.Payload, &req); err != nil {
			slog.Error("capa.cmd.quote_request.payload_failed",
				"client", env.ClientID,
				"error", err)
			return
		}
		...
		if err := c.svc.HandleQuoteRequest(msgCtx, env, req); err != nil {
			slog.Error("capa.cmd.quote_request.handle_failed",
				"client", env.ClientID,
				"error", err)
		}
	})
	...
	subT, err := c.nc.Subscribe(tradeSubject, func(msg *nats.Msg) {
		var env model.Envelope
		if err := json.Unmarshal(msg.Data, &env); err != nil {
			slog.Error("capa.cmd.trade_execute.unmarshal_failed", "error", err)
			return
		}
		var cmd model.TradeCommand
		if err := json.Unmarshal(env.Payload, &cmd); err != nil {
			slog.Error("capa.cmd.trade_execute.payload_failed",
				"client", env.ClientID,
				"error", err)
			return
		}
		...
		if err := c.svc.HandleTradeExecute(msgCtx, env, cmd); err != nil {
			slog.Error("capa.cmd.trade_execute.handle_failed",
				"client", env.ClientID,
				"error", err)
		}
	})
	...
	slog.Info("capa.command_consumer.subscribed",
