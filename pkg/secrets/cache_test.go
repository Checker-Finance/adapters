...
// helper: creates a sample Credentials struct
...
func TestCache_PutAndGet(t *testing.T) {
	cache := NewCache[Credentials](2 * time.Second)
	key := "tenantA|client1|RIO"
...
func TestCache_Expiration(t *testing.T) {
	cache := NewCache[Credentials](500 * time.Millisecond)
	key := "tenantA|client1|RIO"
	cache.Put(key, sampleCreds())
...
func TestCache_Bust(t *testing.T) {
	cache := NewCache[Credentials](5 * time.Second)
	key := "tenantA|client1|RIO"
	cache.Put(key, sampleCreds())
...
func TestCache_CleanupExpired(t *testing.T) {
	cache := NewCache[Credentials](200 * time.Millisecond)
	key1 := "tenantA|client1|RIO"
	key2 := "tenantA|client2|RIO"
