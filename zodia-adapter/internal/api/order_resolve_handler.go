...
// OrderResolverService fetches and normalizes a live transaction from Zodia.
type OrderResolverService interface {
	ResolveTransaction(ctx context.Context, clientID, tradeID string) (*model.TradeConfirmation, error)
}
...
// from Zodia, and syncs the result to the legacy database.
...
// from Zodia, syncs the result to the legacy database, and returns the resolved state.
func (h *OrderResolveHandler) ResolveOrder(c *fiber.Ctx) error {
	...
	// 2. Resolve the provider transaction (trade) ID via the RFQ.
	...
	// 3. Fetch live status from Zodia and map to canonical model.
	trade, err := h.service.ResolveTransaction(ctx, qrec.ClientID, txID)
	if err != nil {
		h.logger.Error("zodia.resolve_order.fetch_failed",
			zap.String("quote_id", quoteID),
			zap.String("tx_id", txID),
			zap.Error(err))
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{"error": "failed to fetch status from Zodia"})
	}

	// 4. Sync to legacy database.
	if err := h.tradeSync.SyncTradeUpsert(ctx, trade); err != nil {
		h.logger.Warn("zodia.resolve_order.sync_failed",
			zap.String("quote_id", quoteID),
			zap.String("tx_id", txID),
			zap.Error(err))
