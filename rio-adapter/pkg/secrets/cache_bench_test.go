...
func BenchmarkCacheGetPut(b *testing.B) {
	cache := NewCache[Credentials](10 * time.Second)
	keyPrefix := "tenantA|client|RIO"

	// Pre-fill some entries to create partial hit ratios
	for i := 0; i < 1000; i++ {
		key := keyPrefix + strconv.Itoa(i)
		cache.Put(key, Credentials{Username: "key-" + strconv.Itoa(i), Password: "secret"})
	}
...
func BenchmarkCacheCleanup(b *testing.B) {
	cache := NewCache[Credentials](1 * time.Second)
	for i := 0; i < 10000; i++ {
